#################################################
# HelloID-Conn-Prov-Target-Topicus-Somtoday-HRMService-Update
# PowerShell V2
#################################################
...
try {
    ...
    if ($null -ne $correlatedAccount) {
        ...
        # However in this case the get-call on uuid used does not return any comparable properties, so always update       
        $outputContext.PreviousData = @{} 
        if ($null -eq $outputContext.AccountReference.UUID) {
            $outputContext.AccountReference.UUID = $correlatedAccount.uuid     
        }
    ...
    switch ($action) {
        'UpdateAccount' {
            Write-Information "Account property(s) required to update: [All properties in the fields configuration]"
...
"@
        ...
        'NoChanges' {
            Write-Information "No changes to Topicus-Somtoday-HRMService account with accountReference: [$($actionContext.References.Account)]"
        ...
        'NotFound' {
            Write-Information "Topicus-Somtoday-HRMService account: [$($actionContext.References.Account)] could not be found, indicating that it may have been deleted"
            $outputContext.Success = $false
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Topicus-Somtoday-HRMService account: [$($actionContext.References.Account)] could not be found, indicating that it may have been deleted"
                    IsError = $true
                })
            break
        }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not update Topicus-Somtoday-HRMService account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditMessage = "Could not update Topicus-Somtoday-HRMService account. Error: $($ex.Exception.Message)"
