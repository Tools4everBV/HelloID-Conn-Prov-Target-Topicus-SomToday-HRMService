...
try {    
    ...
    if ($null -ne $p.primarycontract.custom.schoolBrinCode) {
        $curVestiging = @{
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($currentSchoolCode)) {       
            $currentSchoolCode = $MainBrincode
        }
        ...
        write-verbose -verbose "found employee $($employeeFound |convertto-json)"
...
"@
    ...
    if (-Not($dryRun -eq $true)) {
        $responseCreatePerson = Invoke-webrequest @splatRestMethodParams -Verbose:$true
    }
    else {
